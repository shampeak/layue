...
class Man extends \app\sys\controller\Base{
    ...
    public function tasklist(request $request)
    {
        ...
        //时间限制到今天昨天
        $dtmark      = date("Y-m-d",strtotime("-30 day"));
...
";
        ...
        $usermap = md('user')->getmap('trueName','uId');
//print_r($usermap);
        foreach( $list as $key=>$value){
            $list[$key]['_mediatype'] = $mediamap[$list[$key]['mediatype']];
            $list[$key]['_xmId'] = isset($xmmap[$list[$key]['xmId']])?$xmmap[$list[$key]['xmId']]:'';
            $list[$key]['_zxmId'] = isset($zxmmap[$list[$key]['zxmId']])?$zxmmap[$list[$key]['zxmId']]:'';
            $list[$key]['_zId'] = isset($zhmap[$list[$key]['zId']])?$zhmap[$list[$key]['zId']]:'';
            $list[$key]['_uId'] = isset($usermap[$list[$key]['uId']])?$usermap[$list[$key]['uId']]:'';
