...
class Fin extends \app\sys\controller\Base{
...
group by dt,xmId,uId,cId,zId,fd";
    ...
    public function dolist($list)
    {
        ...
        foreach($list as $key=>$value){
            ...
            if(isset($value['cId'])){
                $list[$key]['fdd'] = $this->getfdd($value);
            }
            ...
            if(isset($value['sumrmb']) && isset($list[$key]['fdd'])) {
                //计算充值
                $fcc = (float)$list[$key]['fdd'];
                $list[$key]['fcc'] = round($list[$key]['sumrmb']/(1+$fcc),2) ;
            }
    ...
    public function getfdd($value){
        $map = $this->getfddmap();

        $cam = md('semcam')->where('cId',$value['cId'])->find();
        $v = '--';
        if($cam){
            $zxm = $cam->zxm;
            if($cam['mediatype']==1){
    ...
    //==============================================
    //账号底点
    public function getfddmap(){
        $zh = md('semzh')->select();
        $map = [];
        foreach($zh as $key=>$value){
            if($value['mediatype']==1){
                $map[$value['id'].'.0'] = $value['rate']?:0;
                $map[$value['id'].'.1'] = $value['rebatexx']?:0;
            }else{
                $map[$value['id']] = $value['rate']?:0;
            }
        }
        return $map;
//        print_r($map);
     ...
     * 我的任务组
     */
    public function xmlist(request $request)
    {
        $key = $request->get('key');
...
//echo $where;
        ...
        $list = md('semxm')->where($where)->select();
